...
CubeMX tabanlı proje (`UKS-Telemetry.ioc`), Makefile ile `arm-none-eabi-gcc` kullanılarak derlenir. STM32 for VSCode extension için `STM32Make.make` mevcut (hedef: `stm32f1x`, cpu: `cortex-m3`, optimizasyon: `Og`).

```bash
make            # build/ dizinine derler (Makefile)
make -f STM32Make.make   # build/debug/ dizinine derler (STM32 for VSCode)
```

**İki build dosyası, iki farklı araçla üretilir — elle senkron tutulmalı:**
