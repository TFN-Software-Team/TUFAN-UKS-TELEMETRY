...
/* _write burada tanimlanmiyor — main.c'deki non-weak tanimlama kullanilir.
 * syscalls.c'de weak _write olsaydi linker hangisini sececegini garanti
 * edemezdik; bu yuzden burada hic tanimlamiyoruz. */
...
clock_t _times(struct tms *buf) { (void)buf; return -1; }
...
{
    (void)file;
    return (int)(unsigned char)__io_getchar();
}
